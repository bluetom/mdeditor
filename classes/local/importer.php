...
namespace tool_mdeditor\local;

use tool_mdeditor\local\mlang\phpparser;
use tool_mdeditor\local\mlang\logstatus;
use tool_mdeditor\local\mlang\langstring;
use core\output\notification;
use stored_file;
...
class importer {
    ...
    private function component_from_filepath(string $filepath) {
        ...
        $result = $DB->get_record('tool_mdeditor_components', ['name' => $componentname]);
    ...
    /**
     * Import an array of strings into the mdeditor tables.
     *
    ...
    private function import_strings(array $strings, stdClass $component): void {
        ...
        foreach ($strings as $newstring) {
            ...
            $mdeditor = $DB->get_record('tool_mdeditor', [
            ...
            if ($this->can_save_string($mdeditor, $newstring, $component)) {
                $mdeditor->local = $newstring->text;
                $mdeditor->timecustomized = $newstring->timemodified;
                $mdeditor->outdated = 0;
                $mdeditor->modified = 1;
                $DB->update_record('tool_mdeditor', $mdeditor);
            }
        }
    }
     ...
     *
     * @param stdClass $mdeditor mdeditor original record
     ...
     */
    private function can_save_string(?stdClass $mdeditor, langstring $newstring, stdClass $component): bool {
        $result = false;

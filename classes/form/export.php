...
/**
 * Creates Formular for mdeditor file export
 *
...
use tool_mdeditor_utils;

/**
 * Formular for mdeditor file export
 *
 * @copyright  2020 Thomas Wedekind <Thomas.Wedekind@univie.ac.at>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class export extends \moodleform {
    ...
    public function definition() {
        ...
        if (!check_dir_exists($langdir) || !count(glob("$langdir/*"))) {
            print_error('nolocallang', 'tool_mdeditor');
        }
        ...
        $select = $mform->addElement('select', 'files', get_string('exportfilter', 'tool_mdeditor'), $fileoptions);
        $select->setMultiple(true);
        $mform->addRule('files', get_string('required'), 'required', null, 'client');
        $mform->setDefault('files', $fileoptions);

        $this->add_action_buttons(true, get_string('export', 'tool_mdeditor'));
    }
}

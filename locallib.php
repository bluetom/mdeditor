...
class tool_mdeditor_utils {
    ...
    public static function checkout($lang, progress_bar $progressbar = null) {
        ...
        $current = $DB->get_records('tool_mdeditor_components', null, 'name', 'name,version,id');
        foreach (self::list_components() as $component) {
            if (empty($current[$component])) {
                ...
                $DB->insert_record('tool_mdeditor_components', $record);
            } elseif ($version = get_component_version($component)) {
                if (is_null($current[$component]->version) or ($version > $current[$component]->version)) {
                    $DB->set_field('tool_mdeditor_components', 'version', $version, array('id' => $current[$component]->id));
                }
            }
        }
        unset($current);
        ...
        foreach ($components as $component) {
            ...
            foreach ($english as $stringid => $stringoriginal) {
                ...
                if (isset($current[$stringid])) {
                    ...
                    if ($needsupdate) {
                        $DB->update_record('tool_mdeditor', $current[$stringid]);
                        continue;
                    }
                    ...
                    $DB->insert_record('tool_mdeditor', $record);
                }
            }
        }

        if (!is_null($progressbar)) {
            $progressbar->update_full(100, get_string('checkoutdone', 'tool_mdeditor'));
        }
    }
    ...
    public static function checkin($lang) {
        ...
        $sql = "SELECT s.*, c.name AS component
                  FROM {tool_mdeditor} s
                  JOIN {tool_mdeditor_components} c ON s.componentid = c.id
                 WHERE s.lang = ?
        ...
        $DB->set_field_select('tool_mdeditor', 'modified', 0, 'lang = ?', array($lang));
...
EOF
    ...
    public static function get_count_of_modified($lang) {
        ...
        return $DB->count_records('tool_mdeditor', array('lang'=>$lang, 'modified'=>1));
    }
    ...
    public static function save_filter(stdclass $data, stdclass $persistant) {
        if (!isset($persistant->tool_mdeditor_filter)) {
            $persistant->tool_mdeditor_filter = array();
        }
        foreach ($data as $key => $value) {
            if ($key !== 'submit') {
                $persistant->tool_mdeditor_filter[$key] = serialize($value);
            }
        }
    }
    ...
    public static function load_filter(stdclass $persistant) {
        $data = new stdclass();
        if (isset($persistant->tool_mdeditor_filter)) {
            foreach ($persistant->tool_mdeditor_filter as $key => $value) {
 ...
 */
class tool_mdeditor_menu implements renderable {
 ...
 */
class tool_mdeditor_translator implements renderable {
    ...
    public function __construct(moodle_url $handler, $lang, $filter, $currentpage = 0) {
        ...
        $sql  = "  FROM {tool_mdeditor_components} c
                   JOIN {tool_mdeditor} s ON s.componentid = c.id
                  WHERE s.lang = :lang
                  AND c.name LIKE 'mod_%'";

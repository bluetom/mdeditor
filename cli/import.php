...
use tool_mdeditor\local\importer;
use core\output\notification;
...
Examples:
\$ sudo -u www-data /usr/bin/php admin/tool/mdeditor/cli/import.php --lang=en --source=mdeditors.zip

\$ sudo -u www-data /usr/bin/php admin/tool/mdeditor/cli/import.php --source=/tmp/mdeditors --checkin

\$ sudo -u www-data /usr/bin/php admin/tool/mdeditor/cli/import.php --lang=en --source=/tmp/mdeditors
...
if (!isset($modes[$modeparam])) {
    cli_error(get_string('climissingmode', 'tool_mdeditor'));
}
$mode = $modes[$modeparam];

if (empty($source)) {
    $source = $CFG->dataroot.'/temp/mdeditor';
}

if (!file_exists($source)) {
    cli_error(get_string('climissingsource', 'tool_mdeditor'));
}
...
foreach ($files as $filepath) {
    ...
    if ($extension == 'zip') {
        if (!$filelang) {
            ...
            if (strrpos($filename, 'mdeditor_') === 0) {
    ...
    if (empty($filelang)) {
        cli_error(get_string('climissinglang', 'tool_mdeditor'));
    }
...
if (!$countfiles) {
    cli_error(get_string('climissingfiles', 'tool_mdeditor'));
}
...
foreach ($langfiles as $lng => $files) {
    ...
    cli_heading(get_string('clifiles', 'tool_mdeditor', $lng));
    ...
    // Import files.
    cli_heading(get_string('cliimporting', 'tool_mdeditor', $modeparam));
    $importer->import($storedfiles);
    // Display logs.
    $log = $importer->get_log();
    if (empty($log)) {
        cli_problem(get_string('clinolog', 'tool_mdeditor', $lng));
    }
    ...
    if ($checkin) {
        ...
        cli_writeln(get_string('savecheckin', 'tool_mdeditor'));
    }
    cli_writeln("");
}

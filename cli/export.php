...
-t, --target            Target to copy the zip files to, default: $CFG->tempdir/mdeditor
...
\$ sudo -u www-data /usr/bin/php admin/tool/mdeditor/cli/export.php

Export just the english files of moodle core and the activity 'quiz' in a subfolder in my home folder:
\$ sudo -u www-data /usr/bin/php admin/tool/mdeditor/cli/export.php --lang='en' --components='moodle,quiz' --target='~/customdir'

EOF;

$dafaulttarget = "$CFG->tempdir/mdeditor/";
...
cli_writeln(get_string('cliexportheading', 'tool_mdeditor'));
$langs = [];
if ($options['lang']) {
...
foreach ($langs as $lang) {
    $filename = $options['target'] . get_string('exportzipfilename', 'tool_mdeditor', ['lang' => $lang]);
    ...
    if ($options['target'] != $dafaulttarget && file_exists($filename) && !$options['overwrite']) {
        cli_problem(get_string('cliexportfileexists', 'tool_mdeditor', $lang));
        continue;
    }
    cli_heading(get_string('cliexportstartexport', 'tool_mdeditor', $lang));
    $langdir = tool_mdeditor_utils::get_localpack_location($lang);
    if (!file_exists($langdir)) {
        // No custom files set for this language set.
        cli_writeln(get_string('cliexportnofilefoundforlang', 'tool_mdeditor', ['lang' => $lang]));
        continue;
    }
    ...
    if ($options['components']) {
        ...
        foreach ($components as $component) {
            ...
            if (file_exists($filepath)) {
                ...
                cli_problem(
                    get_string('cliexportfilenotfoundforcomponent', 'tool_mdeditor', ['lang' => $lang, 'file' => $filepath])
                );
            }
        }
    } else {
    ...
    if (empty($filelist)) {
        cli_problem(get_string('cliexportnofilefoundforlang', 'tool_mdeditor', ['lang' => $lang]));
        continue;
    }
    if ($zipper->archive_to_pathname($filelist, $filename)) {
        cli_writeln(get_string('cliexportzipdone', 'tool_mdeditor', $filename));
    } else {
        cli_problem(get_string('cliexportzipfail', 'tool_mdeditor', $filename));
    }
}
